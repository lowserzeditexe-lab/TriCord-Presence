 ...
 *
 * Vérification du Sec-WebSocket-Accept renvoyé par le serveur : c'est le
 * base64 du SHA-1 de "<key envoyée>258EAFA5-E914-47DA-95CA-C5AB0DC85B11"
 * (GUID fixe défini par la RFC). Sans ce contrôle, un proxy transparent
 * peut renvoyer un "HTTP/1.1 101" arbitraire et le client accepterait n'
 * importe quel upstream. En pratique le contrôle de cert TLS empêche
 * déjà cette attaque, mais la RFC en fait un MUST côté client.
 * ---------------------------------------------------------------------- */
#define WS_GUID "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"

static bool ws_compute_expected_accept(const char *client_key_b64, char *out, size_t outSize) {
    /* concat : client_key_b64 + GUID (les deux sont ASCII) */
    unsigned char buf[64 + sizeof(WS_GUID)];
    size_t klen = strlen(client_key_b64);
    if (klen + strlen(WS_GUID) >= sizeof(buf)) return false;
    memcpy(buf, client_key_b64, klen);
    memcpy(buf + klen, WS_GUID, strlen(WS_GUID));

    unsigned char digest[20];
#if defined(MBEDTLS_VERSION_MAJOR) && MBEDTLS_VERSION_MAJOR >= 3
    /* mbedtls 3.x : mbedtls_sha1() renvoie int. */
    if (mbedtls_sha1(buf, klen + strlen(WS_GUID), digest) != 0) return false;
#else
    /* mbedtls 2.x : mbedtls_sha1() est deprecated (void) ; mbedtls_sha1_ret()
     * renvoie int et n'est pas marqué deprecated dans devkitPro 3ds-mbedtls
     * 2.28. C'est la variante utilisée par Rosalina (Luma3DS libs/mbedtls). */
    if (mbedtls_sha1_ret(buf, klen + strlen(WS_GUID), digest) != 0) return false;
#endif
    size_t olen = 0;
    if (mbedtls_base64_encode((unsigned char *)out, outSize, &olen, digest, sizeof(digest)) != 0) return false;
    out[olen] = '\0';
    return true;
}

/* Recherche insensible à la casse d'un header (RFC 7230 §3.2 : les noms de
 * headers HTTP sont case-insensitive). Retourne un pointeur vers la valeur
 * (après ": ", trim gauche) ou NULL si absent. Modifie temporairement
 * respBuf pour couper la valeur au CRLF. */
static char *http_header_value(char *respBuf, const char *name) {
    size_t nameLen = strlen(name);
    for (char *p = respBuf; *p; p++) {
...
static bool ws_handshake(gw_t *g, const char *host) {
    ...
    /* Vérification RFC 6455 §4.1 : Sec-WebSocket-Accept == base64(SHA1(key + GUID)).
     * On travaille sur une copie locale car http_header_value() écrit un '\0'
     * au CRLF (destructif sur resp[]). */
    char expected[40];
    if (!ws_compute_expected_accept((const char *)key_b64, expected, sizeof(expected))) {
        GW_LOG("SHA1/base64 échoué (build de Sec-WebSocket-Accept attendu)");
        return false;
    }
    char *got_accept = http_header_value(resp, "Sec-WebSocket-Accept");
    if (!got_accept) { GW_LOG("réponse 101 sans Sec-WebSocket-Accept -> proxy défectueux ?"); return false; }
    if (strcmp(got_accept, expected) != 0) {
        GW_LOG("Sec-WebSocket-Accept invalide (attendu=%s, reçu=%s)", expected, got_accept);
        return false;
    }
    return true;
}
...
static void gateway_thread_main(void *arg) {
    ...
    while (s_threadRunning && !g->fatal) {
        /* Veille active : si aucun token n'est actuellement disponible (soit
         * TriCord pas encore connecté au démarrage, soit déconnexion en cours
         * de route), on n'essaie pas d'ouvrir une session ; on dort 5 s puis
         * on refait un tour. discordGatewayRefreshTokenIfChanged() (appelé
         * périodiquement par main.c) écrira s_token dès que TriCord aura un
         * compte, et le prochain tour de boucle enchaînera sur run_session().
         *
         * On protège la lecture par s_presenceLock (le même mutex qui protège
         * l'écriture dans refresh_token) pour ne pas voir un token à moitié
         * réécrit — même si la fenêtre de race pratique est étroite. */
        gw_mutex_lock(&s_presenceLock);
        bool haveToken = (s_token[0] != '\0');
        gw_mutex_unlock(&s_presenceLock);
        if (!haveToken) {
            GW_LOG("veille : pas de token TriCord actif -> attente 5s");
            gw_sleep_ms(5000);
            continue;
        }
...
Result discordGatewayInit(void) {
    ...
    /* On charge le token si possible, mais son ABSENCE n'est plus fatale :
     * le thread réseau (gateway_thread_main) reste alors en "veille active"
     * jusqu'à ce que discordGatewayRefreshTokenIfChanged() écrive s_token
     * (typiquement quand l'utilisateur se connecte dans TriCord APRÈS le
     * démarrage du sysmodule). main.c appelle refresh toutes les 15 s. */
    rc = loadToken(s_token, sizeof(s_token));
    if (R_FAILED(rc)) {
        GW_LOG("Aucun token disponible au démarrage -> veille active (attente de TriCord)");
        s_token[0] = '\0';
    }
 ...
 *
 * Politique de fermeture / reconnexion :
 *   - Changement de compte : le session_id courant appartient à l'ancien
 *     compte -> pas de RESUME possible. On force resume_on_reconnect=false,
 *     on remet session_id/seq à zéro, on demande une fermeture propre avec
 *     un code hors-1000/1001 (4000 "reconnect needed") pour que Discord
 *     n'invalide pas préventivement la nouvelle IDENTIFY, et le thread
 *     réseau se reconnectera après backoff en envoyant un IDENTIFY neuf
 *     avec le nouveau token (déjà écrit dans s_token sous mutex).
 *   - Déconnexion TriCord (accounts absent/illisible) : idem, mais on
 *     efface aussi s_token. Le thread réseau détecte s_token vide au tour
 *     suivant et retombe en "veille active" (attente de token, sleep 5s)
 *     au lieu de mourir sur 4004. Dès que TriCord reconnecte un compte,
 *     le prochain refresh remplit s_token et le thread reprend.
 *
 * Verrou :
 *   - On réutilise s_presenceLock pour protéger s_token et les champs de
 *     session (session_id/seq/resume_on_reconnect) ; le thread réseau ne
 *     les lit qu'en début de session (send_identify/send_resume), donc la
 *     race est courte et bien encadrée par want_close.
 * ---------------------------------------------------------------------- */
bool discordGatewayRefreshTokenIfChanged(void) {
    char candidate[256];
    Result rc = loadToken(candidate, sizeof(candidate));
    bool disconnect = R_FAILED(rc);

    /* Comparaison en temps constant (défense en profondeur, même si l'attaquant
     * local n'a de toute façon rien à gagner à mesurer un timing ici). */
    size_t n = sizeof(s_token);
    unsigned diff = disconnect ? 1u : 0u;
    if (!disconnect) {
        for (size_t i = 0; i < n; i++) diff |= (unsigned)(candidate[i] ^ s_token[i]);
    }
    if (diff == 0) return false; /* rien à faire, on garde la session courante */

    if (disconnect) {
        GW_LOG("TriCord déconnecté (accounts absent/illisible, rc=%d) -> fermeture de la session courante", rc);
    } else {
        GW_LOG("Changement de compte détecté côté TriCord -> reconnexion avec nouveau token");
    }
    ...
    if (disconnect) {
        ...
        snprintf(s_token, sizeof(s_token), "%s", candidate);
    }
    /* Le session_id/seq précédent appartient à l'ancien compte : pas de
     * RESUME possible. On repart sur un IDENTIFY neuf. */
    s_gw.session_id[0] = '\0';
    s_gw.resume_host[0] = '\0';
    s_gw.has_seq = false;
    s_gw.seq = 0;
    s_gw.resume_on_reconnect = false;
    s_gw.ready = false;
    /* Demande de fermeture propre au thread réseau : la boucle run_session()
     * détecte want_close, sort de la poll, queue un CLOSE 4000 (code >1000,
     * donc pas de "session invalidée" côté Discord — pertinent pour le cas
     * "TriCord se reconnecte tout de suite après") puis rend la main. */
    s_gw.want_close = true;
    gw_mutex_unlock(&s_presenceLock);
    return true;
}
...
/* Test unitaire hôte du calcul Sec-WebSocket-Accept — vecteur canonique
 * RFC 6455 §1.3 : SHA1("dGhlIHNhbXBsZSBub25jZQ==" + GUID) en base64
 * = "s3pPLMBiTxaQ9kYGzzhZRbK+xOo=". */
bool discordGatewayTestWsAccept(const char *client_key_b64, char *out, size_t outSize) {
    return ws_compute_expected_accept(client_key_b64, out, outSize);
}
#endif
