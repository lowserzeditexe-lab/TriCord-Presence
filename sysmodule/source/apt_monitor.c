...
//
// Plan B (si NS refuse GetAppletInfo à un process non-applet, cf. TODO ci-
// dessous) : svcGetProcessList pour énumérer les process, puis
// svcGetProcessInfo(handle, type=0x10001) — c'est l'extension kernel de
// Luma3DS (Luma3DS libraries/libctru/source/system/allocateHeaps.c n'a rien
// à voir, la vraie source est k11_extension/source/svc/GetProcessInfo.c
// où type 0x10001 renvoie le programId du process). Le premier process dont
// le programId a pour hi-part 0x00040000 (application native) ou 0x00040002
// (démo, système embarqué) est le jeu au premier plan sur 3DS classique —
// un seul slot Application peut tourner en même temps.
//
// SVCs supplémentaires nécessaires (déjà déclarés dans tricord_presenced.rsf) :
//   svcGetProcessList = 0x65, svcOpenProcess = 0x33.
//   svcGetProcessInfo = 0x2B (déjà présent).
//
// TODO(hardware): non validé sur console.
//   - Le plan A (APT:GetAppletInfo) peut être refusé par NS parce que notre
//     process n'est pas enregistré comme applet — c'est justement pour ça
//     que ce plan B existe.
//   - Le plan B suppose que le type=0x10001 de svcGetProcessInfo est bien
//     l'extension Luma (elle existe depuis Luma3DS v8+ mais son numéro peut
//     changer dans un fork). Le sysmodule est de toute façon lié à Luma3DS
//     par sa présence dans /luma/sysmodules/, donc dépendre d'une extension
//     Luma est acceptable.
//   - Sur Citra-Azahar, svcGetProcessInfo type 0x10001 n'existe pas (Citra
//     n'implémente pas l'extension Luma) : ce plan B ne fonctionnera pas
//     en émulation. Plan A doit fonctionner sur Citra.
Result aptMonitorGetActiveTitleId(u64 *outTitleId) {
    u64 tid = 0;
    bool registered = false;
    Result rc = aptGetAppletInfo(APPID_APPLICATION, &tid, &registered);
    if (R_SUCCEEDED(rc) && registered && tid != 0) {
        *outTitleId = tid;
        return 0;
    }

    /* Plan B : parcours de la liste de processus. On tolère TOUTE erreur du
     * plan A (pas juste "not registered") : si NS refuse notre appel, on
     * essaie le plan B avant de rendre les armes. */
    u32 pidList[64];
    s32 pidCount = 0;
    Result rcList = svcGetProcessList(&pidCount, pidList, (s32)(sizeof(pidList) / sizeof(pidList[0])));
    if (R_FAILED(rcList)) {
        /* Ni plan A ni plan B accessibles -> on remonte l'erreur du plan A
         * si elle existait, sinon celle du plan B. */
        return R_FAILED(rc) ? rc : rcList;
    }

    for (s32 i = 0; i < pidCount; i++) {
        Handle procHandle = 0;
        if (R_FAILED(svcOpenProcess(&procHandle, (u32)pidList[i]))) continue;

        /* svcGetProcessInfo type 0x10001 = programId (extension Luma k11).
         * out est un s64 dans libctru mais on l'utilise comme u64. */
        s64 programId = 0;
        Result rcInfo = svcGetProcessInfo(&programId, procHandle, 0x10001);
        svcCloseHandle(procHandle);
        if (R_FAILED(rcInfo)) continue;

        /* Un titre "application" a hi-part 0x00040000 (cartouche/eShop) ou
         * 0x00040002 (démo/embarqué). Filtrer élimine les sysmodules et le
         * HOME Menu (hi 0x00040030 / 0x00040001). */
        u32 tidHi = (u32)((u64)programId >> 32);
        if (tidHi == 0x00040000u || tidHi == 0x00040002u) {
