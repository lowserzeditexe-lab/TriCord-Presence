...
/* Vecteur canonique RFC 6455 §1.3 : la clé "dGhlIHNhbXBsZSBub25jZQ==" doit
 * produire "s3pPLMBiTxaQ9kYGzzhZRbK+xOo=" comme Sec-WebSocket-Accept. */
static int test_ws_accept(void) {
    char got[40] = {0};
    const char *expected = "s3pPLMBiTxaQ9kYGzzhZRbK+xOo=";
    if (!discordGatewayTestWsAccept("dGhlIHNhbXBsZSBub25jZQ==", got, sizeof(got))) {
        printf("ws-accept KO: fonction retourne false\n");
        return 1;
    }
    if (strcmp(got, expected) != 0) {
        printf("ws-accept KO: attendu='%s', obtenu='%s'\n", expected, got);
        return 1;
    }
    printf("ws-accept RFC 6455 §1.3: OK (%s)\n", got);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    if (test_ws_accept()) return 2;
    if (R_FAILED(discordGatewayInit())) { printf("gateway init erreur (allocation ?)\n"); return 1; }
